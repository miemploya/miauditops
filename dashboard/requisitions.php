                ...
                <div class="glass-card rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-lg overflow-hidden">
                    ...
                    <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button @click="showForm = !showForm; if(!showForm) resetForm(); $nextTick(() => lucide.createIcons())" :class="showForm ? 'from-red-500 to-rose-600 shadow-red-500/30' : 'from-indigo-500 to-violet-600 shadow-indigo-500/30'" class="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r text-white text-xs font-bold rounded-xl shadow-lg hover:scale-[1.02] transition-all">
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <template x-if="editingReqId"><span class="px-3 py-1.5 bg-amber-100 text-amber-700 text-[10px] font-bold rounded-lg">Editing</span></template>
                                    <button @click="createRequisition()" :class="editingReqId ? 'from-amber-500 to-orange-600 shadow-amber-500/30' : 'from-indigo-600 to-violet-700 shadow-indigo-500/30'" class="px-8 py-2.5 bg-gradient-to-r text-white text-xs font-black rounded-xl shadow-lg hover:scale-[1.02] transition-all" x-text="editingReqId ? 'Update Requisition' : 'Submit Requisition'"></button>
                                </div>
                            </div>
                        </div>
                    </div>
                                            ...
                                            <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-indigo-50/50 dark:hover:bg-slate-800/30 cursor-pointer" @click="toggleExpand(r.id)">
                                                ...
                                                <td class="px-3 py-2.5 text-center" @click.stop>
                                                    <div class="flex items-center justify-center gap-1">
                                                        ...
                                                        <template x-if="r.status === 'submitted' && r.requested_by == userId"><button @click="editRequisition(r)" class="p-1 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 rounded-lg transition-all" title="Edit Requisition"><i data-lucide="pencil" class="w-3 h-3 text-blue-500"></i></button></template>
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center gap-4 text-xs text-slate-400">
                                            ...
                                            <template x-if="r.updated_at"><span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-[10px] font-bold"><i data-lucide="pencil" class="w-2.5 h-2.5"></i> Modified <span x-text="r.updated_at?.substring(0,16).replace('T',' ')"></span></span></template>
                                        </div>
                                    </div>
...
function reqApp() {
    return {
        ...
        resetForm() { this.editingReqId = null; this.reqForm = { department:'', purpose:'', priority:'medium', items:[{product_id:'', description:'', quantity:1, unit_price:0, old_price:null, source:'', source_label:''}] }; },
        ...
        async createRequisition() {
            ...
            fd.append('action', this.editingReqId ? 'update' : 'create');
            if (this.editingReqId) fd.append('requisition_id', this.editingReqId);
        ...
        async editRequisition(req) {
            ...
            try {
                ...
                if (r.success && r.items.length > 0) {
                    ...
                    this.reqForm.department = req.department;
                    this.reqForm.purpose = req.purpose;
                    this.reqForm.priority = req.priority;
                    this.reqForm.items = r.items.map(i => ({
                        product_id: i.product_id || 'custom',
                        description: i.product_name || i.description,
                        quantity: parseFloat(i.quantity),
                        unit_price: parseFloat(i.unit_price),
                        old_price: null, source: '', source_label: ''
                    }));
                    this.showForm = true;
                    this.$nextTick(() => lucide.createIcons());
                }
            } catch(e) { alert('Failed to load requisition items'); }
        },
