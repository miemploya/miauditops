function pnlApp() {
    return {
        ...
        showCreateModal: false, showCatalogEditor: false, saving: false,
        stockCatalog: [], aiReportText: '', _chartInstances: {}, collapsedDepts: {}, pnlChartType: 'bar', openingLocked: false,
        prevPnl: { month: '', year: '', totalRevenue: 0, cos: 0, grossProfit: 0, totalOpex: 0, totalOther: 0, operatingProfit: 0, netProfit: 0 },
        prevPnlLoaded: false,
        rollupType: 'Q1', rollupYear: new Date().getFullYear(), rollupLoading: false,
        pnlPreparedBy: 'MIAUDITOPS', pnlReportStatus: 'draft',
        ...
        get stmtCOS() {
            ...
            return this._aggregateCOS(filtered);
        },
        ...
        async fetchPrevPnl() {
            try {
                ...
                if (d.success && d.has_previous) {
                    this.prevPnl = { month: d.month, year: d.year, totalRevenue: +d.totalRevenue || 0, cos: +d.cos || 0, grossProfit: +d.grossProfit || 0, totalOpex: +d.totalOpex || 0, totalOther: +d.totalOther || 0, operatingProfit: +d.operatingProfit || 0, netProfit: +d.netProfit || 0 };
                    this.prevPnlLoaded = true;
                    this.toast('Previous month P&L loaded');
                } else {
                    this.prevPnlLoaded = false;
                    this.prevPnl = { month: '', year: '', totalRevenue: 0, cos: 0, grossProfit: 0, totalOpex: 0, totalOther: 0, operatingProfit: 0, netProfit: 0 };
                }
        ...
        loadPeriodData(pid) {
            ...
            if (!this.revenueItems.length) this.revenueItems = [{ label: '', amount: 0 }];
        ...
        async saveCOS() {
            ...
            fd.append('report_id', this.activeReport.id); fd.append('period_id', this.activePeriodId);
            // Combine COS arrays — skip opening if not first period (opening is locked)
            let all = [
                ...
                ...this.cosClosing.map(i => ({ label: i.label, entry_type: 'closing', sub_entries: [{ qty: +i.qty || 0, unit_cost: +i.unit_cost || 0 }], amount: (+i.qty || 0) * (+i.unit_cost || 0) }))
            ];
        ...
        async saveCatalog() {
            ...
            if (d.success) {
                await this.loadCatalog();
                // Refresh closing stock from new catalog
                if (this.activePeriodId) this.loadPeriodData(this.activePeriodId);
        ...
        generateAIReport() {
            ...
            let sep = '\u2550'.repeat(50), dash = '\u2500'.repeat(40), nl = '\n';
            let recs = '';
            if (p.netProfit < 0) recs += '\ud83d\udd34 URGENT: Operating at a loss. Action needed:\n   - Review pricing strategy upward\n   - Cut non-essential expenses\n   - Negotiate better supplier terms\n';
            if (p.grossMargin < 30) recs += '\ud83d\udfe1 Gross margin below 30%. Consider:\n   - Renegotiating supplier contracts\n   - Reviewing product mix for higher-margin items\n   - Reducing waste and spoilage\n';
            if (p.expenseRatio > 35) recs += '\ud83d\udfe1 Expenses consume over 35% of revenue:\n   - Audit each expense category\n   - Consider process automation\n   - Consolidate vendor contracts\n';
            if (p.netMargin >= 15) recs += '\ud83d\udfe2 Strong net margin. Recommendations:\n   - Reinvest profits into growth areas\n   - Build cash reserves\n   - Consider expansion opportunities\n';
            if (this.cosClosing.length > 0) recs += '\n\u2022 Total closing stock valuation: ' + this.fmt(c.closingStock) + ' across ' + this.cosClosing.filter(i => +i.qty > 0).length + ' items\n';

            this.aiReportText = 'AI FINANCIAL ANALYSIS REPORT' + nl + sep + nl
                + 'Client: ' + (r.client_name || r.title) + nl
                + 'Period: ' + this.monthNames[r.report_month - 1] + ' ' + r.report_year + nl
                + 'Industry: ' + r.industry.toUpperCase() + nl
                + 'Generated: ' + new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) + nl
                + nl + '1. EXECUTIVE SUMMARY' + nl + dash + nl
                + 'Overall Financial Health: ' + health + nl + nl
                + 'The business generated total revenue of ' + this.fmt(p.totalRevenue) + ' during the reporting period with a net ' + (p.netProfit >= 0 ? 'profit' : 'loss') + ' of ' + this.fmt(Math.abs(p.netProfit)) + '. The gross margin stands at ' + p.grossMargin.toFixed(1) + '%, while the net margin is ' + p.netMargin.toFixed(1) + '%.' + nl
                + nl + '2. REVENUE ANALYSIS' + nl + dash + nl
                + '\u2022 Total Revenue: ' + this.fmt(p.totalRevenue) + ' across ' + this.stmtRevenue.length + ' stream(s)' + nl
                + '\u2022 Top Revenue Source: ' + topRev.label + ' contributing ' + this.fmt(topRev.amount) + ' (' + (p.totalRevenue > 0 ? (topRev.amount / p.totalRevenue * 100).toFixed(1) : '0') + '% of total)' + nl
                + '\u2022 Revenue is ' + (this.stmtRevenue.length > 1 ? 'diversified across multiple streams, reducing dependency risk' : 'concentrated in a single stream \u2014 consider diversification') + nl
                + nl + '3. COST EFFICIENCY' + nl + dash + nl
                + '\u2022 Cost of Sales: ' + this.fmt(p.cos) + ' (' + p.cogsRatio.toFixed(1) + '% of revenue)' + nl
                + '\u2022 Opening Stock: ' + this.fmt(c.opening) + nl
                + '\u2022 Add: Purchases: ' + this.fmt(c.purchases) + nl
                + '\u2022 Less: Closing Stock: ' + this.fmt(c.closingStock) + nl
                + '\u2022 ' + (p.cogsRatio > 65 ? '\u26a0\ufe0f COGS ratio exceeds 65% \u2014 review supplier costs and pricing' : p.cogsRatio > 50 ? 'COGS ratio acceptable but monitor for increases' : '\u2705 COGS ratio is well-controlled') + nl
                + nl + '4. EXPENSE MANAGEMENT' + nl + dash + nl
                + '\u2022 Total Operating Expenses: ' + this.fmt(p.totalOpex) + nl
                + '\u2022 Total Other Expenses: ' + this.fmt(p.totalOther) + nl
                + '\u2022 Combined Expense Ratio: ' + p.expenseRatio.toFixed(1) + '% of revenue' + nl
                + '\u2022 Highest Expense: ' + topExp.label + ' at ' + this.fmt(topExp.amount) + nl
                + '\u2022 ' + (p.expenseRatio > 40 ? '\u26a0\ufe0f Expense ratio is high \u2014 identify areas for cost reduction' : '\u2705 Expense ratio is within healthy range') + nl
                + nl + '5. KEY RECOMMENDATIONS' + nl + dash + nl
                + recs + nl
                + sep + nl
                + 'Note: This report is auto-generated based on financial data. Review and modify as needed before presenting to stakeholders.' + nl
                + 'Powered by MIAUDITOPS \u2014 Miemploya Audit Support';
        ...
        exportPnlPDF() {
            ...
            const pageHeader = (num, title) => `
                <div style="display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #000;padding-bottom:8px;margin-bottom:24px">
                    <h3 style="font-size:14px;font-weight:900;color:#000;text-transform:uppercase;letter-spacing:0.5px">${String(num).padStart(2, '0')}. ${title}</h3>
            ...
            const pageFooter = (pg) => `
                <div style="position:absolute;bottom:18mm;left:20mm;right:20mm;border-top:1px solid #f1f5f9;padding-top:8px;display:flex;justify-content:space-between;opacity:.5">
                    <p style="font-size:8px;font-weight:900;text-transform:uppercase;letter-spacing:2px">Generated by MIAUDITOPS</p>
                    <p style="font-size:8px">${genDate}</p>
                </div>`;
            ...
            <div class="page" style="display:flex;flex-direction:column;justify-content:space-between">
                ...
                <div style="padding:60px 0;text-align:center">
                    <div style="display:inline-block;padding:6px 20px;border:2px solid #000;margin-bottom:28px">
                        <span style="font-size:11px;font-weight:900;letter-spacing:5px;text-transform:uppercase">End of Month</span>
                    </div>
            ...
            <div class="page">
                ...
                <tbody>
                    ...
                    <tr style="background:#000"><td style="padding:16px 10px;font-weight:900;color:#fff;font-size:14px;border-radius:8px 0 0 8px">Net Profit / (Loss)</td><td style="padding:16px 10px;text-align:right;font-weight:900;font-size:14px;border-radius:0 8px 8px 0;color:${pnl.netProfit >= 0 ? '#34d399' : '#fca5a5'}">${f(pnl.netProfit)}</td></tr>
                </tbody></table>
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:32px">
                    <div style="background:#000;border-radius:12px;padding:16px;text-align:center"><p style="font-size:18px;font-weight:900;color:#f59e0b">${f(pnl.totalRevenue)}</p><p style="font-size:9px;font-weight:700;color:rgba(255,255,255,.5);text-transform:uppercase;margin-top:4px">Total Revenue</p></div>
                    <div style="background:#000;border-radius:12px;padding:16px;text-align:center"><p style="font-size:18px;font-weight:900;color:#f59e0b">${f(pnl.cos)}</p><p style="font-size:9px;font-weight:700;color:rgba(255,255,255,.5);text-transform:uppercase;margin-top:4px">Cost of Sales</p></div>
            ...
            const noteTitle = (num, title) => `
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px"><div style="width:22px;height:22px;border-radius:6px;background:#000;display:flex;align-items:center;justify-content:center"><span style="font-size:9px;font-weight:900;color:#f59e0b">${num}</span></div><span style="font-size:10px;font-weight:900;color:#000;text-transform:uppercase;letter-spacing:1px">${title}</span></div>`;
            ...
            const buildColumnTable = (dataByPeriod, allItems, totalLabel, totalColor) => {
                ...
                // Header row
                let html = '<table style="width:100%;border-collapse:collapse;font-size:9px"><thead><tr style="background:#000">';
                html += '<th style="padding:7px 6px;text-align:left;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px;font-size:8px">Item</th>';
                ps.forEach(p => {
                    const d1 = p.date_from ? p.date_from.slice(5) : '';
                    const d2 = p.date_to ? p.date_to.slice(5) : '';
                ...
                html += '<th style="padding:7px 6px;text-align:right;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px;font-size:8px">Total</th></tr></thead><tbody>';
                // Data rows
                ...
                // Total row
                html += `<tr style="background:#f8fafc;border-top:2px solid #000"><td style="padding:7px 6px;font-weight:900;color:#000">${totalLabel}</td>`;
                let grandTotal = 0;
                ps.forEach(p => {
            ...
            <div class="page">
                ...
                <div style="margin-bottom:24px">
                    ...
                    ${revItems.length > 0 && this.periods.length > 0 ? buildColumnTable(revByPeriod, revItems, 'Total Revenue', '#059669') : `<table style="width:100%;border-collapse:collapse;font-size:10px"><tbody>${this.stmtRevenue.map(i => `<tr style="border-bottom:1px solid #f1f5f9"><td style="padding:6px 0;color:#334155">${esc(i.label)}</td><td style="padding:6px 0;text-align:right;font-weight:700;color:#000">${f(i.amount)}</td></tr>`).join('')}<tr style="background:#f8fafc;border-top:2px solid #000"><td style="padding:8px 0;font-weight:900;color:#000">Total Revenue</td><td style="padding:8px 0;text-align:right;font-weight:900;color:#059669">${f(pnl.totalRevenue)}</td></tr></tbody></table>`}
                </div>
                ${cosBlock}
                ...
                <div style="margin-bottom:24px">
                    ...
                    ${expItems.length > 0 && this.periods.length > 0 ? buildColumnTable(expByPeriod, expItems, 'Total Operating Expenses', '#2563eb') : `<table style="width:100%;border-collapse:collapse;font-size:10px"><tbody>${this.stmtOpex.map(i => `<tr style="border-bottom:1px solid #f1f5f9"><td style="padding:6px 0;color:#334155">${esc(i.label)}</td><td style="padding:6px 0;text-align:right;font-weight:700;color:#000">${f(i.amount)}</td></tr>`).join('')}<tr style="background:#f8fafc;border-top:2px solid #000"><td style="padding:8px 0;font-weight:900;color:#000">Total Operating Expenses</td><td style="padding:8px 0;text-align:right;font-weight:900;color:#2563eb">${f(pnl.totalOpex)}</td></tr></tbody></table>`}
                </div>
                ${otherBlock}
                ${pageFooter(3)}
            </div>`;
            ...
            const metricCard = (val, label, color) => `<div style="background:#000;border-radius:12px;padding:20px;text-align:center"><p style="font-size:22px;font-weight:900;color:${color || '#f59e0b'}">${val}</p><p style="font-size:9px;font-weight:700;color:rgba(255,255,255,.5);text-transform:uppercase;margin-top:6px">${label}</p></div>`;
            ...
            const ratioRow = (label, value, unit, color, desc) => {
                ...
                return `<tr style="border-bottom:1px solid #f1f5f9"><td style="padding:10px 12px"><span style="font-size:11px;font-weight:700;color:#1e293b">${label}</span><br><span style="font-size:8px;color:#94a3b8">${desc}</span></td><td style="padding:10px 12px;text-align:right"><span style="font-size:14px;font-weight:900;color:${color}">${value.toFixed(1)}${unit}</span></td><td style="padding:10px 12px;width:120px"><div style="height:8px;background:#f1f5f9;border-radius:999px;overflow:hidden"><div style="height:100%;border-radius:999px;background:${color};width:${barW}%"></div></div></td><td style="padding:10px 12px;text-align:center"><span style="font-size:8px;font-weight:800;color:${statusColor};background:${statusColor}15;padding:2px 8px;border-radius:4px;text-transform:uppercase">${status}</span></td></tr>`;
            };
            const page4 = `
            <div class="page">
                ...
                <div style="border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;margin-bottom:20px">
                    ...
                    <thead><tr style="background:#f8fafc"><th style="padding:8px 12px;text-align:left;font-size:8px;font-weight:800;color:#94a3b8;text-transform:uppercase">Ratio</th><th style="padding:8px 12px;text-align:right;font-size:8px;font-weight:800;color:#94a3b8;text-transform:uppercase">Value</th><th style="padding:8px 12px;font-size:8px;font-weight:800;color:#94a3b8;text-transform:uppercase">Indicator</th><th style="padding:8px 12px;text-align:center;font-size:8px;font-weight:800;color:#94a3b8;text-transform:uppercase">Health</th></tr></thead>
                    <tbody>
            ...
            if (this.prevPnlLoaded) {
                ...
                <div class="page">
                    ...
                    <div style="border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">
                        <table style="width:100%;border-collapse:collapse;font-size:11px">
                        <thead><tr style="background:#000"><th style="padding:10px 12px;text-align:left;font-size:9px;font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:.5px">Line Item</th><th style="padding:10px 12px;text-align:right;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase">${esc(curLabel)}</th><th style="padding:10px 12px;text-align:right;font-size:9px;font-weight:900;color:#94a3b8;text-transform:uppercase">${esc(prevLabel)}</th><th style="padding:10px 12px;text-align:right;font-size:9px;font-weight:900;color:#fff;text-transform:uppercase">Variance</th><th style="padding:10px 12px;text-align:right;font-size:9px;font-weight:900;color:#fff;text-transform:uppercase">% Change</th></tr></thead>
                        <tbody>
            ...
            const page5 = `
            <div class="page">
                ${pageHeader('04', 'Stock Valuation Report')}
                ${closingItems.length > 0 ? `
                <table style="width:100%;border-collapse:collapse;font-size:11px">
                <thead><tr style="background:#000"><th style="padding:10px 8px;text-align:left;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px">Item</th><th style="padding:10px 8px;text-align:right;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px">Unit Cost</th><th style="padding:10px 8px;text-align:center;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px">Packs</th><th style="padding:10px 8px;text-align:center;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px">Pieces</th><th style="padding:10px 8px;text-align:center;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px">Total Qty</th><th style="padding:10px 8px;text-align:right;font-size:9px;font-weight:900;color:#f59e0b;text-transform:uppercase;letter-spacing:.5px">Stock Value</th></tr></thead>
                <tbody>${stockHTML}</tbody>
                <tfoot><tr style="background:#000"><td style="padding:12px;font-weight:900;color:#fff;font-size:10px" colspan="5">Total Closing Stock Valuation</td><td style="padding:12px;text-align:right;font-weight:900;color:#f59e0b;font-size:12px">${f(cosCalc.closingStock)}</td></tr></tfoot>
                </table>` : '<div style="padding:40px;text-align:center;color:#94a3b8;border:1px dashed #e2e8f0;border-radius:12px">No stock valuation data available</div>'}
                ${pageFooter(5)}
            </div>`;
            ...
            const page6 = `
            <div class="page">
                ${pageHeader('05', 'Financial Infographics')}
            ...
            const buildBarChart = (title, items) => {
                ...
                let rows = items.map((item, idx) => {
                    ...
                    return `<div style="display:flex;align-items:center;margin-bottom:6px"><span style="font-size:9px;font-weight:600;color:#475569;width:110px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(item.label)}</span><div style="flex:1;margin:0 6px;height:14px;background:#f1f5f9;border-radius:999px;overflow:hidden"><div style="height:100%;border-radius:999px;background:${c};width:${Math.min(100, barW)}%"></div></div><span style="font-size:8px;font-weight:700;color:${c};width:36px;text-align:right">${pct}%</span><span style="font-size:9px;font-weight:700;color:#334155;width:72px;text-align:right">${f(item.amount)}</span></div>`;
                }).join('');
                return `<div style="border:1px solid #e2e8f0;border-radius:12px;padding:20px"><p style="font-size:10px;font-weight:900;color:#000;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">${title}</p>${rows}</div>`;
            };
            const buildPieChart = (title, items) => {
                ...
                const legend = slices.map(s => `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><div style="width:10px;height:10px;border-radius:3px;background:${s.color};flex-shrink:0"></div><span style="font-size:9px;font-weight:600;color:#475569;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(s.label)}</span><span style="font-size:8px;font-weight:700;color:${s.color}">${s.pct.toFixed(1)}%</span><span style="font-size:8px;font-weight:600;color:#94a3b8">${f(s.amount)}</span></div>`).join('');
                return `<div style="border:1px solid #e2e8f0;border-radius:12px;padding:20px"><p style="font-size:10px;font-weight:900;color:#000;text-transform:uppercase;letter-spacing:1px;margin-bottom:16px">${title}</p><div style="display:flex;gap:16px;align-items:center"><div style="width:120px;height:120px;border-radius:50%;background:conic-gradient(${gradient});flex-shrink:0;box-shadow:0 2px 8px rgba(0,0,0,0.1)"></div><div style="flex:1;min-width:0">${legend}</div></div></div>`;
            };
            ...
            const aiText = (this.aiReportText || '').replace(/\n/g, '<br>');
            const page8 = `
            <div class="page">
                ${pageHeader('07', 'Recommendation Report')}
                <div style="border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">
            ...
            const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>P&L Statement — ${esc(clientName)}</title><style>${css}</style></head><body>${page1}${page2}${page3}${page4}${pageMoM}${page5}${page6}${page7}${page8}</body></html>`;
