            ...
            <div x-show="currentTab === 'expenses'" x-transition>
                ...
                <div class="glass-card rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-lg overflow-hidden mb-6">
                    <div class="px-6 py-3 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-amber-500/10 to-transparent flex items-center justify-between">
                        ...
                        <button type="button" @click="showCatForm = !showCatForm" class="text-xs font-bold text-white bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 flex items-center gap-1.5 px-4 py-2 rounded-xl shadow-md hover:shadow-lg transition-all">
                            <i data-lucide="plus-circle" class="w-4 h-4"></i>
                            <span x-text="showCatForm ? 'Cancel' : 'Add Category'"></span>
                        </button>
                    </div>
                    <!-- Inline Add Category (collapsible) -->
                    <div x-show="showCatForm" x-transition class="px-6 py-4 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-700">
                        ...
                        <div class="border-t border-amber-200/50 dark:border-amber-700/50 pt-4">
                            <h4 class="text-[10px] font-bold text-amber-700 uppercase mb-2">Manage Categories</h4>
                            <div class="max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    <template x-for="c in categories" :key="c.id">
                                        <div class="flex items-center justify-between p-2 bg-white/50 dark:bg-slate-900/50 rounded-lg border border-amber-100 dark:border-amber-800 hover:border-amber-300 transition-colors">
                                            <div class="flex flex-col">
                                                <span class="text-xs font-bold text-slate-800 dark:text-slate-200" x-text="c.name"></span>
                                                <span class="text-[9px] uppercase font-semibold text-slate-400" x-text="c.type.replace('_',' ')"></span>
                                            </div>
                                            <button @click="deleteCategory(c.id)" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-all" title="Delete Category">
...
function financeApp() {
    return {
        ...
        async deleteCategory(id) {
            if (!confirm('Are you sure you want to delete this category? It will only be deleted if it has no associated expenses.')) return;
            const fd = new FormData();
            ...
            if (r.success) {
                ...
                if (this.expenseForm.category_id == id) this.expenseForm.category_id = '';
            } else { alert(r.message); }
        },
        async saveExpense() {
