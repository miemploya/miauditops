...
<body class="font-sans bg-slate-950 text-white min-h-screen" x-data="ownerApp()" x-init="init()">
    ...
    <main class="ml-64 min-h-screen">
        ...
        <div x-show="tab==='deleted'" x-cloak>
            <div class="flex items-center gap-3 mb-5">
                <div class="relative flex-1 max-w-sm">
                    <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500"></i>
                    <input type="text" x-model="deletedSearch" placeholder="Search deleted accounts..." class="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-red-500">
                </div>
                <div class="flex items-center gap-2 text-xs text-slate-400">
                    <i data-lucide="info" class="w-3.5 h-3.5"></i>
                    <span>Deleted companies' users can re-register with the same email</span>
                </div>
            </div>

            <div class="bg-slate-800/40 rounded-2xl border border-slate-700/50 overflow-hidden">
                <template x-if="filteredDeleted().length === 0">
                    <div class="p-12 text-center">
                        <i data-lucide="check-circle" class="w-10 h-10 text-emerald-600 mx-auto mb-2"></i>
                        <p class="text-sm text-slate-500">No deleted accounts</p>
                    </div>
                </template>

                <!-- Table Header -->
                <div class="grid grid-cols-7 gap-3 px-5 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider bg-slate-800/60" x-show="filteredDeleted().length > 0">
                    <span>Company</span>
                    <span>Code</span>
                ...
                <div class="divide-y divide-slate-700/50">
                    <template x-for="d in filteredDeleted()" :key="d.id">
                        <div class="grid grid-cols-7 gap-3 px-5 py-3 items-center text-sm hover:bg-slate-800/60 transition-colors">
                            <span class="font-semibold text-white truncate" x-text="d.name"></span>
                            <span class="font-mono text-xs text-slate-400" x-text="d.code"></span>
                            <span class="text-xs text-slate-400 truncate" x-text="d.owner_email || '—'"></span>
                            <span class="text-xs">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold"
                                      :class="{'bg-blue-500/10 text-blue-400': d.plan_name==='professional', 'bg-purple-500/10 text-purple-400': d.plan_name==='enterprise', 'bg-slate-500/10 text-slate-400': !d.plan_name || d.plan_name==='free' || d.plan_name==='starter'}"
                                      x-text="(d.plan_name || 'free').charAt(0).toUpperCase() + (d.plan_name || 'free').slice(1)"></span>
                            </span>
                            <span class="text-xs text-slate-400" x-text="d.user_count"></span>
                            <span class="text-xs text-red-400" x-text="new Date(d.deleted_at).toLocaleDateString('en-GB', {day:'numeric',month:'short',year:'numeric'})"></span>
                            <div class="flex justify-end gap-2">
                                <button @click="restoreCompany(d)" class="px-3 py-1.5 text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded-lg hover:bg-emerald-500/20 transition-all flex items-center gap-1">
...
function ownerApp() {
    return {
        ...
        async deleteCompany(id) {
            ...
            if (!confirm(`Are you sure you want to delete "${name}"? All users will be soft-deleted and can re-register.`)) return;
            const r = await this.api('delete_company', { id });
            this.companies = this.companies.filter(c => c.id !== id);
            this.loadStats();
            if (r.message) alert(r.message);
        },

        async loadDeletedCompanies() {
            const r = await this.api('list_deleted_companies');
            if (r.success) this.deletedCompanies = r.data;
            this.$nextTick(() => lucide.createIcons());
        },

        async restoreCompany(d) {
            if (!confirm(`Restore "${d.name}" and all its users?`)) return;
            const r = await this.api('restore_company', { id: d.id });
            if (r.success) {
                this.deletedCompanies = this.deletedCompanies.filter(c => c.id !== d.id);
                if (r.message) alert(r.message);
                this.loadStats();
            }
        },
        ...
        filteredDeleted() {
            const q = this.deletedSearch.toLowerCase();
            return this.deletedCompanies.filter(d =>
                !q || d.name?.toLowerCase().includes(q) || d.code?.toLowerCase().includes(q) || d.owner_email?.toLowerCase().includes(q)
            );
        },
